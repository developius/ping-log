...
Finally we repeat that but just for 'today' but for all sources.

````{r millisecondsByDateTimeToday, warning = FALSE, fig.cap = "HubPi results for the last two days (if we have any)"}
# repeat for yesterday & today
today <- Sys.Date()

library(scales)
ggplot(allPinger_DT[r_date == today], 
       aes(x = r_datetime, y = milliseconds)) +
  geom_point(aes(color = host, col = "Response time (ms)")) +
  theme(legend.position = "bottom") +
  theme(legend.title = element_blank()) +
