...
for(f in uniqueSources) {
  ...
  temp_DT$error <- ifelse(grepl("From",temp_DT$milliseconds),
                             "ping error: `From'",
                             temp_DT$error)
  ...
  temp_DT[, milliseconds_numeric := as.numeric(milliseconds)]
  
  # force any remaining NA milliseconds without error flags to have error flags
  temp_DT$error <- ifelse(is.na(temp_DT$milliseconds_numeric) &
                            temp_DT$error == "OK",
                             "ping error: unknown",
                             temp_DT$error)
  # update milliseconds & remove
  temp_DT[, milliseconds := milliseconds_numeric]
  temp_DT[, milliseconds_numeric := NULL]
...
We should have either error codes or 'OK'. We can recode the NAs as unknown errors and retry.
...
Repeat that graph but for the last two days and only for the router Pi to be able to inspect the most recent results.

````{r millisecondsByDateTimeRecent, warning = FALSE, fig.cap = "HubPi results for the last two days"}
...
And again but just for today (`r Sys.Date()`).

````{r millisecondsByDateTimeToday, warning = FALSE, fig.cap = "HubPi results for today"}
# repeat for yesterday & today
today <- Sys.Date()

ggplot(allPinger_DT[r_date == today & label %like% "BThub"], 
       aes(x = r_datetime, y = milliseconds)) +
  geom_point(aes(color = host, col = "Response time (ms)")) +
  theme(legend.position = "bottom") +
  theme(legend.title = element_blank()) +
...
Guess when the squids got home?
