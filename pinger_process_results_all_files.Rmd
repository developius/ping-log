...
for(f in uniqueSources) {
  ...
  # Get the file
  # this may throw errors and warnings which we really should handle nicely
  temp_DT <- fread(f)
...
# if record = OK but no milliseconds then unknown ping error
...
````{r, echo=FALSE}
kable(
  table(allPinger_DT$label, allPinger_DT$error, useNA = "always")
)
````
...
````{r, echo=FALSE}
kable(
  table(allPinger_DT$r_hour, allPinger_DT$error, useNA = "always")
)
...
ggplot(allPinger_DT[allPinger_DT$error!="OK"], aes(x = r_hour)) + 
  geom_bar(aes(fill = error, col = "Count")) +
